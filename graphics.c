...
int drawClip(double x0, double y0, double x1, double y1, struct graphics *g, double bright, double hue, long long meta, int antialias, double thick, struct tilecontext *tc, int m, struct dump_meta *data) {
	double xmin = -1 - thick;
	double ymin = -1 - thick;
...
void drawPixel(double x, double y, struct graphics *g, double bright, double hue, long long meta, struct tilecontext *tc, int m, struct dump_meta *data) {
...
void drawBrush(double x, double y, struct graphics *g, double bright, double brush, double hue, long long meta, int gaussian, struct tilecontext *tc, int m, struct dump_meta *data) {
...
#define MULT 9
	...
	for (xx = 0; xx < width; xx++) {
		for (yy = 0; yy < width; yy++) {
			drawPixel(x + xx, y + yy, g, brushbytes[yy * width + xx] * bright / (MULT * MULT), hue, meta, tc, m, data);
		}
	}
}
